    ...
    public sealed class CaseDatabaseDebugReport : MonoBehaviour
    {
        ...
        private static void ValidateFactReferences(List<string> errors, DatabaseManager databaseManager)
        {
            foreach (var fact in databaseManager.FactDatabase.FactById.Values)
            {
                ...
                {
                    if (!databaseManager.StatementDatabase.StatementById.ContainsKey(dialogueId) &&
                        !databaseManager.TruthDatabase.InterrogationLayerById.ContainsKey(dialogueId))
                    {
                        errors.Add($"Fact '{fact.factId}' references missing source statement/layer '{dialogueId}'.");
                    }
                }
                ...
                foreach (var locationId in fact.scope?.relatedLocationIds ?? new List<string>())
                {
                    ...
                    {
                        errors.Add($"Fact '{fact.factId}' contains a blank location id.");
                    }
                }
            }
        }

        private static void ValidateStatementReferences(List<string> errors, DatabaseManager databaseManager)
        {
            foreach (var topic in databaseManager.StatementDatabase.TopicById.Values)
            {
                if (!databaseManager.NpcDatabase.NpcById.ContainsKey(topic.npcId))
                {
                    errors.Add($"Statement topic '{topic.topicId}' references missing npc '{topic.npcId}'.");
                }
                ...
                foreach (var entry in topic.entries ?? new List<StatementEntryData>())
                {
                    ...
                    {
                        errors.Add($"Statement topic '{topic.topicId}' contains a blank statement id.");
                        continue;
                    }

                    if (!string.IsNullOrWhiteSpace(entry.replacesStatementId) &&
                        !databaseManager.StatementDatabase.StatementById.ContainsKey(entry.replacesStatementId))
                    {
                        errors.Add(
                            $"Statement '{entry.statementId}' replaces missing statement '{entry.replacesStatementId}'.");
                    }
                }
            }
        }
            ...
            TruthInterrogationLayerData layer)
        {
            ...
            foreach (var topicId in layer.relatedStatementTopicIds ?? new List<string>())
            {
                if (!databaseManager.StatementDatabase.TopicById.ContainsKey(topicId))
                {
                    errors.Add($"Interrogation layer '{layer.layerId}' references missing statement topic '{topicId}'.");
                }
            }
        }
            ...
            string requirementId)
        {
            ...
            if (databaseManager.EvidenceDatabase.EvidenceById.ContainsKey(requirementId) ||
                ...
                databaseManager.StatementDatabase.StatementById.ContainsKey(requirementId) ||
            ...
            IEnumerable<TruthInterrogationLayerData> layers)
        {
            foreach (var layer in layers ?? new List<TruthInterrogationLayerData>())
            {
                ...
                AppendResolvedTopicList(report, databaseManager, "statementTopics", layer.relatedStatementTopicIds);
            }
        }
        ...
        private static void AppendResolvedTopicList(
            StringBuilder report,
            DatabaseManager databaseManager,
            string label,
            IEnumerable<string> topicIds)
        {
            ...
            var hasAny = false;
            foreach (var topicId in topicIds ?? new List<string>())
            {
                hasAny = true;
                if (databaseManager.StatementDatabase.TryGetTopic(topicId, out var topic))
                {
                    report.Append($"{topicId}({topic.displayName}); ");
                    continue;
                }

                report.Append($"{topicId}(MISSING); ");
            }
